...
exports.requestToCreateDonationActivityOrder = async (req, res) => {
  ...
  if (!req.body) {
    ...
    const encodedABI = contract.methods.requestToCreateOrderFromDonationActivity(newContractId,donation_activity_address,retailer_address,receipt_url,total_amount).encodeABI();
    ...
    User.queryHostFindByAddress(validated_host_address, (err, current_user) => {
      if (err) {
        ...
        privKey = currentUserJson.private_key
        console.log(
            `Attempting to create Donation Activity Order Request from ${validated_host_address}}`
        );
